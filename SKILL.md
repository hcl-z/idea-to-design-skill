...
description: Use when a user has a rough idea for a web or app product and needs help turning it into frontend UI and interaction design: pages, flows, layouts, states, and visual direction before implementation.
---
...
- what screens or pages exist
- what the user sees on each page
- how the user moves between pages
- what actions, states, feedback, and hierarchy the interface should expose
- what visual system supports that interface

It should NOT design:
- backend architecture
- API contracts
- database schema
- domain model or service boundaries
- internal automation logic
- class structure, component architecture, or implementation code

If the user starts mixing product idea, UI design, and system implementation together, separate them clearly. Do the UI and interaction design in this skill, and leave code or architecture planning for later.
...
3. **Ask clarifying questions** — one at a time, focused on purpose, users, features, pages, layout, and style
4. **Propose 2-3 approaches** — with trade-offs and your recommendation
5. **Present design** — in sections scaled to complexity, get user approval after each section
6. **Write design doc** — save the validated design brief and prepare it for review
7. **Spec review loop** — re-check the written design and fix issues before handing it off
8. **User reviews written spec** — ask the user to review the spec file before moving on
9. **Transition to implementation** — invoke writing-plans to create an implementation plan if needed
...
**The terminal state is invoking writing-plans.** Do NOT invoke implementation skills before the design is approved.
...
- Do not ask implementation-detail questions unless they directly affect visible UI behavior

## The Process

**Understanding the idea:**

- Check the current project state first if files, notes, or prior context exist
...
- Do not drift into data structures, backend orchestration, or internal system behavior unless it changes the UI
...
- For each key page, define the goal, structure, main modules, and key actions
- Define important visible states: default, empty, loading, success, error, disabled, onboarding, or confirmation states when relevant
- Define key interactions: search, filter, sort, create, edit, delete, confirm, retry, expand, compare, and navigate when relevant
...
- Propose 2-3 different product or UX directions with trade-offs
- Present the recommended option first with reasoning
- Trade off simplicity, clarity, product ambition, and visual personality
...
> "Design brief written to `<path>`. Please review it and let me know if you want any changes before we move on to implementation planning."
...
- Invoke the writing-plans skill to create a detailed implementation plan if the user wants to build from the brief
...
For the design-system portion, follow the spirit of `DESIGN.md`: a single markdown-based source of design intent that an AI coding agent can read directly. Keep it human-readable, version-friendly, and implementation-oriented. Focus on the design decisions that actually matter for this product instead of mechanically filling a giant template.

The output must NOT turn into a technical design document. Do not specify backend logic, API routes, schema design, data pipelines, or engineering architecture except where a visible UI dependency must be called out as an assumption.

## Output Template

Always include these sections in the final document. Keep the overall structure stable, but let the design-system content behave more like `DESIGN.md`: concise, explicit, and only as detailed as the product needs.
...
- Separate pages into:
  - Core pages
  - Supporting pages
  - System or state-specific pages if needed
...
- Include only the sections needed for this product, typically from:
  - colors
  - typography
  - spacing
  - components
...
When information is incomplete, make the smallest reasonable assumption and mark it clearly.

## Example Of Good Page Detail

Instead of:

- Dashboard page with a clean modern layout

Write:

- Dashboard
  - Goal: give returning users a fast overview of active work and next actions
  - Layout: top summary bar, left filter rail, central task list, right-side insights panel
  - Main modules: KPI strip, recent items, pinned actions, alerts
  - Primary actions: create item, resume task, filter by status
